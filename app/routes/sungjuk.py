...
async def sungjukok(req: Request, name: str = Form(...) , kor: int = Form(...), eng: int = Form(...), mat: int = Form(...)):
    tot = kor + eng + mat
    avg = tot / 3
    grd = '가'
    if (avg >= 90): grd = '수'
    ...
    sj = SungJuk(name=name, kor=kor, eng=eng, mat=mat, tot=tot, avg=avg, grd=grd)
